...
pqivf_probe<T, dist_type>::pqivf_probe(int dimension, int topk):probe<T, dist_type>(){
    ...
    this->res = (pair<Tout, idx_t>*)malloc(this->res_cache_size);
}
...
template<typename T,
        DistanceType dist_type>
pqivf_probe<T, dist_type>::create_gallery(gallery<T, dist_type> ** ga_ptr){
...
inline void get_value(int8_t* data, pair<T, idx_t>* code_book, int code_len, int qid, int block, pair<T, idx_t>* res){
    for (int i = 0; i < block; ++i){
        ...
        for (int j = 0; j < code_len; ++j)
            v += code_book[j];
        res[i].first = v;
        res[qid].second = qid + i;
    }
}
template<typename T,
        DistanceType dist_type>
pqivf_probe<T, dist_type>::query(const T * const x, const int n, gallery<T, dist_type> * ga, Tout *sims, uint32_t *idx){
        ...
        for (int i = 0; i < n; ++i){
            for (int j = 0; j < this->cq_num; ++j)
                for (int k = 0; k < c_ga->data[j].size(); ++k)
                    ans[i].push_back(std::make_pair(vec_dis<T, dist_type>(x + 1LL * i * this->dimension, c_fa->data[j].data() + 1LL * k * this->dimension, this->dimension), k));
    ...
    for (int i = 0; i < n; i += this->max_batch){
        ...
        for (int j = 0; j < std::min(this->max_batch, n-i); ++j){
            for (int cq_id = 0; cq_id < this->select_cq; ++cq_id){
                ...
                int num = c_ga->block_num[cq_res[cq_id]];
                for (int vec_id = 0, vec_id < num; vec_id += this->max_block){
                    get_value<Tout>(this->data.data() + vec_id * dimension, pq_res, this->code_len, i + j, std::min(num, this->max_block - vec_id), &this->res[(i + j) * this->max_block + vec_id]);
...
pqivf_probe<T, dist_type, matrix_type>::query_with_uids(const T* const x, const int n, gallery<T, dist_type> * ga, uint32_t *uids, const int m, Tout *sims, uint32_t *idx){
