...
int get_ans_2(){
    ...
    rsearch::probe<float>* p = rsearch::create_probe<float>(dimension, topk, rsearch::EUCLIDEAN, rsearch::X86_PQIVF_MULTI_THREAD);
...
int test_compare(){
    ...
    fin2.close();*/
    /*random_data.resize(200000 * dimension);
    for (int i = 0; i < 200000; ++i){
        int k = rand() % n;
        //std::cout << k << std::endl;
        memcpy(random_data.data() + i * dimension, vec.data() + 1LL * k * dimension, dimension * sizeof(float));
    }*/
    if (get_ans_2() != 0)
        return -1;
